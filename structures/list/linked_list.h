...
namespace structures
{
    ...
    inline Structure& LinkedList<T>::assign(Structure& other)
    {
        if (this != &other) {
            ...
            clear();
            LinkedListItem<T>* otherItem = otherLinkedList.first_;
    ...
    {
        LinkedListItem<T>* newLinkedListItem = new LinkedListItem<T>(data);
        if (size_ == 0) {
    ...
    inline void LinkedList<T>::insert(const T& data, int index)
    {
        if (size_ == index) {
            ...
            if (index == 0) {
                ...
                LinkedListItem<T>* previousLinkedListItem = getItemAtIndex(index - 1);
    ...
    {
        LinkedListItem<T>* currentLinkedListItem = first_;
        for (size_t i = 0; i < size_; i++) {
            if (currentLinkedListItem->accessData() == data) {
                return i;
            }
    ...
    {
        if (size_ != 0) {
            LinkedListItem<T>* itemToDelete = first_;

            while (itemToDelete != nullptr) {
                LinkedListItem<T>* nextItem = itemToDelete->getNext();
                delete itemToDelete;
