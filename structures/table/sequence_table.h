...
namespace structures
{
	...
	{
		TableItem<K, T>* foundItem = findTableItem(key);

		if (foundItem != nullptr) {
			return foundItem->accessData();
		}

		throw::std::logic_error("Couldnt find item with key!");
	}
	...
	inline void SequenceTable<K, T>::insert(const K& key, const T& data)
	{
		if (containsKey(key) == true) {
			throw::std::logic_error("Key already in sequence table, cannot insert in list!");
		}
		else {
	...
	inline T SequenceTable<K, T>::remove(const K& key)
	{
		...
		if (item != nullptr) {
			...
			throw::std::logic_error("Cannot remove item from sequence table, key is not in list!");
		}
	}
